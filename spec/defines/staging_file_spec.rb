...
describe 'staging::file', type: :define do
  on_supported_os.each do |os, facts|
    context "on #{os}", focus: facts[:osfamily] == 'windows' do
      let(:facts) { facts }
      ...
      describe 'when deploying via local', unless: facts[:osfamily] == 'windows' do
        let(:title) { 'sample.tar.gz' }
        let :params do
          {
      ...
      describe 'when deploying via Windows local', if: facts[:osfamily] == 'windows' do
        let(:title) { 'sample.tar.gz' }
        let(:params) do
          {
      ...
      describe 'when deploying via http' do
        ...
        it do
          ...
          is_expected.to contain_exec('/opt/staging/sample.tar.gz').with(command: 'curl  -f -L -o /opt/staging/sample.tar.gz http://webserver/sample.tar.gz',
                                                                        path: %r{\A/usr/local/bin:/usr/bin:/bin},
                                                                        environment: nil,
                                                                        cwd: '/opt/staging',
